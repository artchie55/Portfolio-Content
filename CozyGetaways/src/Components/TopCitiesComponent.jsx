...
import TopCitiesCarousels from "../Elements/TopCitiesCarousels";
          ...
          <div className='hidden md:flex'>
            {filters.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className={`flex flex-col items-center transition-opacity duration-300 group ${item.text === 'TopCities' ? 'opacity-100' : 'opacity-55 active:opacity-100'}`}
              >
                <img className='h-[3.5rem] mx-[2rem]' src={item.src} alt={item.text} />
                <div className='text-[1rem] relative'>
                  {item.text}
                  <span className={`absolute bottom-0 left-0 h-[0.1rem] bg-black transition-all duration-300 ease-in-out ${item.text === 'TopCities' ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </div>
              </a>
            ))}
          </div>
